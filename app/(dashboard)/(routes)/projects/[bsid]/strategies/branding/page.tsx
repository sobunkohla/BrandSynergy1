...
import StrategyCardBranding from "@/components/custom/strategy-card-branding";
...
const BusinessStrategiesPage = async ({
    ...
    if (!userId) {
          ...
          const brandingstrategy = await db.brandingStrategy.findMany({
            where: {
...
<Heading title="Branding Strategies" description="get access to all your branding strategies" icon={FaHandshake} bgColor="bg-orange-200" iconColor="text-orange-600"/>
...
<Link href={`/projects/${params.bsid}/strategies/`}>
        ...
        <Link href={`/projects/${params.bsid}/strategies/branding/new`}>
          <Button className="flex items-center text-white hover:scale-110 active:scale-95 transition-all bg-orange-600 ">
            <PlusIcon size={20} className="mr-2" /> Create Branding Strategy
          </Button>
        </Link>
</div>
      ...
      {brandingstrategy.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 gap-4  pt-12 ">
          {brandingstrategy.map((strategy) => (
            <StrategyCardBranding strategy={strategy } marketspaceId={marketspace.id} key={strategy.id} />
          ))}
        </div>
      )}
    </div>
